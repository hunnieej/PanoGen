...
class SphereDiffusion(nn.Module):
    def __init__(self, device, rf_version='2.0', hf_key=None):
        ...
        if self.rf_version == 'SANA':
            ...
            self.vae = AutoencoderKL.from_pretrained(model_key, subfolder="vae", torch_dtype=torch.bfloat16, token="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").to(self.device)
            self.tokenizer_1 = CLIPTokenizer.from_pretrained(model_key, subfolder="tokenizer", token="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")
            self.tokenizer_2 = CLIPTokenizer.from_pretrained(model_key, subfolder="tokenizer_2", token="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")
            self.tokenizer_3 = T5Tokenizer.from_pretrained(model_key, subfolder="tokenizer_3", token="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")
            self.text_encoder_1 = CLIPTextModel.from_pretrained(model_key, subfolder="text_encoder", torch_dtype=torch.bfloat16, token="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").to(self.device)
            self.text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(model_key, subfolder="text_encoder_2", torch_dtype=torch.bfloat16, token="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").to(self.device)
            self.text_encoder_3 = T5EncoderModel.from_pretrained(model_key, subfolder="text_encoder_3", torch_dtype=torch.bfloat16, token="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").to(self.device)
            self.unet = SD3Transformer2DModel.from_pretrained(model_key, subfolder="transformer", torch_dtype=torch.bfloat16, token="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").to(self.device)
            self.scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(model_key, subfolder="scheduler", token="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")
        # UNet 기반 
        elif self.rf_version == 'xl':
        ...
        if self.rf_version in ['SANA', '3.5']:
            try:
                sig = inspect.signature(self.unet.forward)
                self._supports_pos_embed = ('pos_embed' in sig.parameters)
            except Exception:
                self._supports_pos_embed = False
        if self._supports_pos_embed:
            print(f"[INFO] Transformer supports external pos_embed: ON")
        elif self.rf_version in ['SANA','3.5']:
            print(f"[WARN] Transformer does not expose pos_embed param; using its internal 2D pos.")
        ...
        with tqdm(total=total_steps, desc='Generating panorama') as pbar:
            for step_idx, t_scalar in enumerate(self.scheduler.timesteps):
                ...
                for tile_idx, tile in enumerate(tiles):
                    ...
                    if self.rf_version in ['SANA','3.5'] and self._supports_pos_embed:
                        Ht, Wt = tile["H"], tile["W"]
                        assert Ht % patch == 0 and Wt % patch == 0, "Ht/Wt must be divisible by patch_size"
                        Hp, Wp = Ht // patch, Wt // patch
                        d_world = compute_patch_directions(Ht, Wt, patch, tile["K"],
                                                           tile["R"], device=self.device,
                                                           dtype=self.unet.dtype)
                        lon = torch.atan2(d_world[:,:,0], d_world[:,:,2])          # [Hp,Wp]
                        lat = torch.asin(d_world[:,:,1].clamp(-1,1))                            # [Hp,Wp]
                        # pos = build_2d_sincos_pe(Hp, Wp, Dm, device=self.device, dtype=self.unet.dtype)  # [1,N,D]
                        pos = build_spherical_sincos_pe(lon, lat, Dm, device=self.device, dtype=self.unet.dtype)  # [1,N,D]
                        cond_kwargs["pos_embed"] = pos.expand(latent_in.shape[0], -1, -1) if use_cfg else pos
